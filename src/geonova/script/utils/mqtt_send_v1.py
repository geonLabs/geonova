...
class PayloadSender:
    def __init__(self, results, img_path):
        ...
        self.img_path = img_path  # ✅ 단일 이미지 파일
    ...
    def recive_result(self):
        for data in self.results:
            ...
            evnet = {
                ...
                "eventType": "SOC",
                "imageID": os.path.basename(self.img_path),  # ✅ 개별 이미지 ID
    ...
    def upload_image(self):
        ...
        try:
            with open(self.img_path, "rb") as image_file:
                ...
                data = {"box_result": self.boxes_result, "timestamp": self.timestamp}
                response = requests.post(self.api_url, headers=headers, files=files, data=data)
                if response.status_code == 200:
                    return {"status": "success", "uploaded": self.img_path}
                else:
                    print(f"Failed to upload {self.img_path}: {response.text}")
        except Exception as e:
            print(f"Error uploading {self.img_path}: {e}")